...
def preroi_filter(k, b):
    ...
    """
    global k_list, b_list, diff_count
    ...
    if np.array(k_list).shape[0]>n:
        new_k = np.average(np.array(k_list)[-n:], weights=k_weights)
        new_b = np.average(np.array(b_list)[-n:], weights=k_weights)
        print new_k, new_b
        if abs(new_k-k)>20 or abs(k-k_list[-n])>50:
            print "line change dtected:  " + str(abs(new_k-k))
            new_k = k_list[-1]
            new_b = b_list[-1]
...
def main(args):
    ...
    global k_list, b_list, f_k_list, f_b_list, count_nan, noedge, preroi_offset, postroi_offset, reach_first_pose, diff_count, point_list
