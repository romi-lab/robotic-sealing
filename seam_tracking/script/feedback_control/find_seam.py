...
def transform_cam_wrt_base(pcd, T_end_effector_wrt_base):
    ...
    T_cam_wrt_end_effector = np.array(  [[ 0.00211918, -0.97207334, -0.23466769,  0.127350972619],
...
def find_orientation(trajectory, pcd, groove, normal):
    ...
    r = R.from_dcm(np.vstack((-y_dir, x_dir, z_dir)).T)
    ...
    for i in range(np.array(points).shape[0]):
        ...
        if i == np.array(points).shape[0]-1:
            ...
            last_pose = Pose()
            #tip position
            last_pose.position.x = points[i][0]
            last_pose.position.y = points[i][1]
            last_pose.position.z = points[i][2]
            #tip orientation
            last_pose.orientation.x = orientation[0]
            last_pose.orientation.y = orientation[1]
            last_pose.orientation.z = orientation[2]
            last_pose.orientation.w = orientation[3]
            pub_last_pose.publish(last_pose)
            print("last pose published")
...
if __name__ == "__main__":
    ...
    pub_last_pose = rospy.Publisher('last_pose', Pose, queue_size=1)
    pub_execute_seam_tracking = rospy.Publisher('execute_seam_tracking', Point, queue_size=1)

    capture_number = 0 

    horizontal_plane = [-1.067366902028219, -0.8888581434832972, -2.384127680455343, 0.12086498737335205, 1.0657631158828735, -1.4749344030963343]
        ...
        if execution == True:
            ...
            if capture == 'q':
                ...
                if not received_ros_cloud is None:
                    ...
                    execute_prompt = raw_input("\n\nenter to execute or q to quit: ")
                    if not execute_prompt == 'q':
                        execute = Point()
                        execute.x = 0
                        execute.y = 0
                        execute.z = 0
                        pub_execute_seam_tracking.publish(execute)
